...
class PlotFunc:
    ...
    def __init__(self, oimg_file, mimg_file, 
                 fstring,
                 sky, skysig, save_name='9999'):

        self.oimg_file = oimg_file
        self.mimg_file = mimg_file
        self.sky = sky
        self.skysig = skysig
    ...
    def plot_profile(self):
        ...
        if not os.path.exists(oimg_file):
            ...
            # Read mask
            f_mask = fitsio.FITS(mimg_file)
            mask = f_mask[0].read()
            f_mask.close()

            model = ma.array(model, mask, fill_value=9999)
            model = model.filled()
            ...
            hist_mask[where(abs(residual) > 12 * skysig)] = 1
            hist_res = ma.masked_array(residual, hist_mask)

        fell = 'E_{}.txt'.format(fstring)
        ...
        oell = 'OE_{}.txt'.format(fstring)
        if os.path.exists(oell):
            data_o = np.genfromtxt(oell, delimiter=' ', names=True)
            sma_o = data_o['sma']		#sma from ellise fitting
            flux_o = data_o['inte']	#Flux at various sma
            flux_err_o =data_o['intee']	#Error in Flux
            mag_o = data_o['mag'] + float(c.mag_zero)#Magnitude at various sma
    ...
    #    rect6 = [0.125, 0.75*0.225 + 0.075, 0.225, 1.5*0.225]
        ...
        try:
            ...
            axUM = plt.axes(rect2)
            im = plt.imshow(model, cmap=cm.jet, 
            ...
            axUR = plt.axes(rect3)
            im = plt.imshow(residual, cmap=cm.jet, 
            ...
            nn, bins, patches = plt.hist(hist_res1d, 50, normed=0)
            nMaxArg = nn.argmax() 
            if(nMaxArg < 16):
                ArgInc = nMaxArg
            else:

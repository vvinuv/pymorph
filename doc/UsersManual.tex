...
\item
Create PSF files. A file contains the list of PSF should be given in config.py (see next section) % either using PyMorph or by some other means.
\item
...
\begin{itemize}
\item \textbf{\red{imagefile:}} As described earlier, if the user is giving a large image frame (Normal mode a.) which include several galaxies to be fitted, then the name of the image should be given here. % This image will be used only if the user are decomposing galaxies in a large frame.
...
\item \textbf{\red{clus\_cata:}} The list of all the objects of interest, i.e. objects the user need to fit. The possible columns in this file are given in the Section: \ref{cluscat} and each column has a name (see Section: \ref{cluscat}). %The program needs at least \textit{gal\_id} or \textit{gimg} to run.
...
\item[] \textbf{crashhandler:} \red{Keep it False} If it is True, then the PyMorph will handle the possible crashes during the pipeline process and try to fix those  errors in the next run. %The details can be found in the Section \ref{crash}
\end{itemize}

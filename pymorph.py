...
"""PyMorph [Py MOrphological Parameters' Hunter], is a pipeline to find the
Morphological parameters of galaxy.
Authors: Vinu Vikram, Yogesh Wadadekar, Ajit K. Kembhavi. 2008 Feb, Alan Meert 2010"""
...
from astropy.io import fits as pyfits   # FIX: pyfits is deprecated, use astropy
import numpy as np
import re
...
sys.path.append(os.path.join(os.path.dirname(__file__), 'pymorph'))
...
def main():
    ...
    # Reading image and weight files
    ...
    try:
        ...
        print("c.components undefined. Assuming bulge+disk model")
        ComP = ['bulge', 'disk']
    if len(ComP) == 0:
        print("No model specified. Assuming bulge+disk model")
        ComP = ['bulge', 'disk']

    # Writing csv header and finding the output parameters
    ParamToWrite = ut.PyMorphOutputParams(c.dbparams, c.decompose)
    
    # Handle the CSV Header separately to ensure it's saved
    write_header = not exists("result.csv")
    if write_header:
        with open("result.csv", "a", newline='') as f_res:
            writer = csv.writer(f_res)
            csvlist = ['%s_%d' % (ParamToWrite[par_key][0], par_key)
                       for par_key in list(ParamToWrite.keys())]
            print("Writing new header to result.csv")
            writer.writerow(csvlist)
    
    # Now open your other files for the processing loop
    f_cat = open(out_cata, 'w')
    f_failed = open('restart.cat', 'w')
    obj_file = open(c.datadir + clus_cata, 'r')
    ...
    for line_j in obj_file:
        ...
        try:
            ...
            try:
                ...
                try:
                    gal_id = pdb["gimg"].split('.')[0]
                except:
                    print("No image or gal_id found in the object catalogue. Exiting")
                    os._exit(0)
            ...
            try:
                ...
                print("No ra2 (minute) is given. Assuming ra is in deg")
                alpha2 = 0
            try:
            ...
            try:
                ...
                if c.repeat == True:
                    ...
                    print("Repeat is false. No possible cfile (galfit config file) is using")
                    cfile = 'None'
            if exists(cfile):
                gimg, oimg, wimg, c.pfile, mimg, cofile = ut.ReadGalfitConfig(cfile)
                gimg = gimg.split('/')[-1]
                oimg = oimg.split('/')[-1]
                wimg = wimg.split('/')[-1]
            ...
            try:
                ...
                print('No ximg is given. Trying to find from the cutout if no ra dec in the image header')
            ...
            try:
                ...
                print('No yimg is given. Trying to find from the cutout if no ra dec in the image header')
                if (c.galcut == True and RaDecInfo == 0):
                    yimg = TY / 2.0
                else:
                    yimg = -9999
            ...
            try:
                ...
                UserGivenPsf = 'None'
            try:
                UserGivenSky = pdb['sky']   # FIX: was pdf['sky'] (typo)
            ...
            for line_s in open(sex_cata, 'r'):
                try:
                    ...
                    if values[0].strip().isdigit():
                        print('Something happened in the pipeline. Check error.log')
                    else:
                        pass
            ...
            try:
                ...
                if exists(str(c.sex_cata) + '.Shallow'):
                    f_sex_shallow = open(str(c.sex_cata) + '.Shallow', 'r')
                ...
                if c.decompose:
                    psffile, distance = ut.HandlePsf(cfile, UserGivenPsf, alpha_j, delta_j)
                ...
                if c.repeat == False:
                    if c.galcut == False:
                        if exists(cutimage):
                            ut.WriteError('The file ' + cutimage + ' exists\n')
                            c.run = 0
                            break
                        try:
                            cut_xcntr, cut_ycntr, SizeX, SizeY, ExceedSize = \
                    ...
                    if c.galcut:
                        if ReSize:
                            ...
                            cut_xcntr, cut_ycntr, SizeX, SizeY, ExceedSize = xcntr, ycntr, TX, TY, 0
                    print('Center of cutimage and exceed size ', cut_xcntr, cut_ycntr, ExceedSize)
                    ...
                    ElliMaskFunc(cutimage, cut_xcntr, cut_ycntr, SizeX, SizeY, good_object, 1)
                    if c.decompose:
                        ut.HandleEllipseTask(cutimage, cut_xcntr, cut_ycntr,
                                             SizeX, SizeY, c.SexSky, 0)
                        MaskFunc(cutimage, cut_xcntr, cut_ycntr, SizeX, SizeY, good_object)
                ...
                try:
                    SexySky, SkyYet, SkyMed, SkyMin, SkyQua, SkySig = \
                        FindYetSky(c.datadir + cutimage, cut_xcntr, cut_ycntr)
                    if SkyMin != 9999:
                        c.SkyMin = SkyYet * 1.0
                        c.SkySig = SkySig * 1.0
                    else:
                ...
                if (c.cas):
                    C, C_err, A, A_err, S, S_err, G, M = \
                        ut.HandleCasgm(cutimage, cut_xcntr, cut_ycntr,
                                       alpha_j, delta_j, z,
                                       SizeX, SizeY, good_object, bxcntr, bycntr)
                else:
                    C, C_err, A, A_err, S, S_err, G, M = \
                        9999, 9999, 9999, 9999, 9999, 9999, 9999, 9999

                for ff in ['BMask.fits', 'MRotated.fits', 'MaskedGalaxy.fits', 'Rotated.fits']:
                ...
                try:
                    Goodness = -9999
                    if os.access('P_' + c.fstring + '.png', os.F_OK):
                        os.remove('P_' + c.fstring + '.png')
                        GoodNess = PlotFunc(outimage, maskimage, cut_xcntr, cut_ycntr,
                                            c.SexSky, c.SkySig)
                ...
                if isset(c.Flag, GetFlag("GALFIT_FAIL")):
                ...
                if ToClean:
                    ClaCli = 'E*fits E*txt G_* I*fits *.pl *.con M_* O*fits O*txt P*png R*html error.log galfit.* Tmp* SO* agm_r* BMask.fits MaskedGalaxy.fits MRotated.fits B.fits GalEllFit.fits AResidual.fits ellip err BackMask.fits'
            ...
            print("something bad happened!!!!\n\n")
            print(traceback.print_exc())
            if len(values) > 0 and values[0].strip().isdigit():
                print('Something happened in the pipeline. Check error.log 2')
            else:
                pass
...
def selectpsf(ImG, CaT):
        ...
        for line in open(CaT, 'r'):
            ...
            try:
                ...
                if float(values[16]) >= c.StarGalProb and \
                   float(values[13]) > AreaOfObj and \
                   float(values[14]) < 50.0:
                    xcntr = float(values[1]) - 1
                    ycntr = float(values[2]) - 1
                    PsfSize = np.floor(float(values[14])) * c.starsize
                    x1 = int(xcntr) + int(PsfSize / 2)
                    x2 = int(xcntr) - int(PsfSize / 2)
                    y1 = int(ycntr) + int(PsfSize / 2)
                    y2 = int(ycntr) - int(PsfSize / 2)
                    ...
                    ra11 = '0' + str(ra1) if ra1 < 10 else str(ra1)
                    ra22 = '0' + str(ra2) if ra2 < 10 else str(ra2)
                    if ra3 < 10:
                    ...
                    if abs(dec1) < 10:
                        dec11 = '-0' + str(abs(dec1)) if dec1 < 0.0 else '+0' + str(abs(dec1))
                    else:
                        dec11 = str(dec1) if dec1 < 0.0 else '+' + str(dec1)
                    dec22 = '0' + str(dec2) if dec2 < 10 else str(dec2)
                    if dec3 < 10:
                    ...
                    psffile = 'psf_' + str(ra11) + str(ra22) + str(ra33) + \
                              str(dec11) + str(dec22) + str(dec33) + '.fits'
                    if psffile in c.psff:
                        ...
                        hdu.header['XCNTR'] = int(xcntr)   # FIX: hdu.header.update deprecated
                        hdu.header['YCNTR'] = int(ycntr)
                        hdu.writeto(psffile)
            except:
                pass
    ...
    while len(c.psff) < 5 and AreaOfObj > 10:
        ...
        AreaOfObj -= 5
    if len(c.psff) == 0:
        manualpsf = input("Unfortunately, NO psf is found. Please enter the psf name >>> ")
    ...
    if c.Interactive:
        ...
        print('Checking Started. You can just visually check the psf.')
        for element in c.psff:
            ...
            if exists(element):
                ...
                print('The psf you have given does NOT exist!!!')
                pass
            write = input("Do you need this psf? ('y' if yes, 'c' to cancel psf checking) ")
            if write == 'y':
                ...
                try:
                    ...
                    pass
        print('\nFinal Checking Started.')
        UrPsfChk = input("Do you want to use your own psf? Enter 'y' or 'n' >>> ")
        if UrPsfChk == 'y':
            UrPsf = input("Enter your psf >>> ")
        ...
        while finish == 0:
            for element in PsfList:
                if exists(element):
                    ...
                    print('The psf you have given does NOT exist!!!')
                    pass
                write = input("Do you REALLY need this psf? ('y' or 'n' or press any key to continue) ")
                if write == 'y':
            ...
            for element in TmpPsfList:
                try:
                    ...
                    pass
            TmpPsfList = []
            fi = input("Finished? ('1' to finish, any other key to continue) ")
            if fi == '0' or fi == '1':
                finish = int(fi)
                if finish == 1:
                    for element in PsfList:
        ...
        for element in c.psff:
            ...
            fff.writelines([str(element), '\n'])
        if fff:
            fff.close()   # FIX: was outside loop with no guard, caused UnboundLocalError if psff empty


def SExtractorConf():
    SEx_DETECT_MINAREA = input("DETECT_MINAREA (6) >>> ")
    try:
        c.SEx_DETECT_MINAREA = int(float(SEx_DETECT_MINAREA))
    except:
    ...
    filter_map = {
        '0': 'default.conv', '': 'default.conv',
        '1': 'gauss_1.5_3x3.conv', '2': 'gauss_2.0_3x3.conv',
        '3': 'gauss_2.0_5x5.conv', '4': 'gauss_2.5_5x5.conv',
        '5': 'gauss_3.0_5x5.conv', '6': 'gauss_3.0_7x7.conv',
        '7': 'gauss_4.0_7x7.conv', '8': 'gauss_5.0_9x9.conv',
    }
    c.SEx_FILTER_NAME = filter_map.get(SEx_FILTER_NAME, 'default.conv')
    SEx_DEBLEND_NTHRESH = input('DEBLEND_NTHRESH (32) >>> ')
    try:
        c.SEx_DEBLEND_NTHRESH = int(float(SEx_DEBLEND_NTHRESH))
    except:
    ...
    try:
        c.SEx_BACK_SIZE = int(float(SEx_BACK_SIZE))
    except:
        c.SEx_BACK_SIZE = 64
    SEx_BACK_FILTERSIZE = input('BACK_FILTERSIZE (3) >>> ')
    try:
        c.SEx_BACK_FILTERSIZE = int(float(SEx_BACK_FILTERSIZE))
    except:
        c.SEx_BACK_FILTERSIZE = 3
    SEx_BACKPHOTO_TYPE = input('BACKPHOTO_TYPE (G)LOBAL/(L)OCAL) >>> ')
    while SEx_BACKPHOTO_TYPE != 'G' and SEx_BACKPHOTO_TYPE != 'L' and SEx_BACKPHOTO_TYPE != '':
        SEx_BACKPHOTO_TYPE = input('BACKPHOTO_TYPE (G)LOBAL/(L)OCAL) >>> ')
    ...
    # FIX: try/except was broken — SEx_BACKPHOTO_THICK only defined inside if block
    if c.SEx_BACKPHOTO_TYPE == 'LOCAL':
        SEx_BACKPHOTO_THICK = input('BACKPHOTO_THICK (24) >>> ')
        try:
            c.SEx_BACKPHOTO_THICK = int(float(SEx_BACKPHOTO_THICK))
        except:
...
def run_SExtractorConf(option, opt, value, parser):
    try:
        ...
        raise Exception("failure in SextractorConf()")
    return
...
if __name__ == '__main__':
    ...
    def FindAndFit():
        if c.findandfit == 1:
            if c.psfselect > 2:
                magmin = input("What is the faintest magnitude galaxy that you want to fit? >>> ")
                try:
                    magmin = float(magmin) * 1.0
                except:
                    magmin = 9999
                magmax = input("What is the brightest magnitude galaxy that you want to fit? >>> ")
                try:
                    magmax = float(magmax) * 1.0
                except:
                    magmax = -9999
                stargal = input("Enter star-galaxy classification (1 for star, 0 for galaxy, 0.8 is a good number) >>> ")
                redshift = input("Enter redshift, if you know >>> ")
                try:
    ...
    try:
        ...
        print(c.imagefile, "I/O error: %s" % str(e))
        os._exit(0)
    ...
    if exists(sex_cata):
        ...
        print('The SExtractor catalogue for your frame is NOT found. '
              'One is being made using the default values.')
        # FIX: broken if/else structure — else was misindented causing SyntaxError
            ...
            for line_j in obj_file:
                try:
                    ...
                    try:
                        ...
                        if exists(c.datadir + 'I' + str(c.rootname) + '_' + str(gal_id) + '.fits'):
                    ...
                    try:
                        ...
                        if exists(c.datadir + 'W' + str(c.rootname) + '_' + str(gal_id) + '.fits'):
                            wimg = 'W' + str(c.rootname) + '_' + str(gal_id) + '.fits'
                        else:
                            wimg = 'None'
                    GiMg = pyfits.open(c.datadir + gimg)
                    headerGiMg = GiMg[0].header
                    c.SEx_GAIN = headerGiMg['GAIN'] if 'GAIN' in headerGiMg else 1
                    GiMg.close()
                    if exists(sex_cata):
                        pass
                    else:
                    ...
                    pass
            obj_file.close()
            AskForUpdate = input("Do you want to update the clus_cata? ('y' for yes) ")
            if AskForUpdate == 'y':

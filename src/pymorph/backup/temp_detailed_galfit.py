...
for input_file in ['G_bar_629409.in', 'G_bar_12.in', 'G_bar_47.in']:
    ...
    mag, rad, n, magbar, rbar, nbar = uniform_params(mag, rad, n=4, nbar=None, 
                                                     number_random=50,
                   delmag=0.5, delr=[0.6, 1.3], deln=[0.5, 1.5],
                   magbar=0.5, rbar=1.2) 


    eb = objects['bulge']['ell']
    ed = objects['disk']['ell']
    ebar = objects['bar']['ell']

    objects['bulge']['mag'] = [m for m in mag]
    objects['disk']['mag'] = [m for m in mag]
    objects['bar']['mag'] = [m for m in magbar]
    objects['bulge']['rad'] = [r for r in rad]
    objects['disk']['rad'] = [r for r in rad]
    objects['bar']['rad'] = [r for r in rbar]
    objects['bulge']['n'] = [n1 for n1 in n]
    objects['bar']['n'] = [n1 for n1 in nbar]
    objects['bulge']['ell'] = [e] *  len(mag)
    objects['disk']['ell'] = [e] *  len(mag)
    objects['bar']['ell'] = [e] *  len(mag)
